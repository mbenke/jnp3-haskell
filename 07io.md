...
Można też użyć jakiegoś GHC online, np. https://www.jdoodle.com/execute-haskell-online
...
Bierze on obliczenie o wyniku (typu) `a` oraz funkcję która na podstawie `a` tworzy obliczenie o wyniku `b` i łączy je w obliczenie o wyniku `b`, np.
...
Dla usprawnienia zapisu oraz dla podkreślenia potencjalnej imperatywności możemy użyć notacji `do`:
...
:pencil: Jak będzie działał program
